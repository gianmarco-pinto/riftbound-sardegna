...
// A CONCLUDED event with registrations but no standings yet = results pending
// (the store enters them late — a Nexus Night, an official tier, is often filled
// a couple of days after). Keep retrying it in newTargets for this many days, then
// give up. An event with ZERO registrations (nobody signed up) is marked done on
// the first fetch — it's not a loading problem, so it's excluded immediately.
const PENDING_RESULT_DAYS = Number(process.env.PENDING_RESULT_DAYS || 4);
...
async function handle(ev) {
  try {
    ...
    transaction(() => {
      ...
      // Mark done (stop re-listing in newTargets) when: we got standings; OR NOBODY
      // registered (participants===0 — not a loading problem, exclude immediately);
      // OR results are still pending after PENDING_RESULT_DAYS (registered but no
      // standings — give the store a few days to enter them, then give up). An event
      // that keeps re-listing forever would burn the 800-event budget and starve
      // real but slightly older events at the bottom of the date-DESC queue.
      if (placed > 0 || participants === 0 || ageDays > PENDING_RESULT_DAYS) markIngested(ev.id);
